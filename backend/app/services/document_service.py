...
class DocumentService:
    """Service for detecting document tampering and forgery with lazy loading."""
    
    def __init__(self):
        """Initialize service without loading model (lazy loading)."""
        self.model_name = "document_model.pt"
        logger.info("DocumentService initialized (lazy loading enabled)")
    ...
    def _load_model_lazy(self) -> Any:
        ...
        def load_fn(model_path: Path, device: torch.device) -> Any:
            """Load document model from checkpoint."""
            logger.info("Loading document model checkpoint...")
            checkpoint = torch.load(model_path, map_location=device)
            
            # Determine checkpoint format
            if isinstance(checkpoint, dict) and not hasattr(checkpoint, 'state_dict'):
                # Check if it's a state_dict or wrapper dict
                ...
                logger.info(f"✅ Document model loaded from state_dict")
                return model
                ...
                model.to(device)
                model.eval()
                logger.info(f"✅ Document model loaded as complete object")
                return model
            else:
                raise TypeError(f"Unrecognized checkpoint type: {type(checkpoint)}")
    ...
    def analyze_document(self, document_bytes, filename: str = "") -> dict:
        ...
        """
        logger.info("=" * 80)
        logger.info("DOCUMENT ANALYSIS START (LAZY LOADING)")
        logger.info("=" * 80)
